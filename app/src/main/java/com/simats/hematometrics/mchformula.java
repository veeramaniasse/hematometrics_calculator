...
public class mchformula extends AppCompatActivity {
    EditText HbInput, rbcCountInput;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mchformula);
        // Initialize UI elements
        HbInput = findViewById(R.id.pcv);
        rbcCountInput = findViewById(R.id.rbc);
    ...
    private void handleResponse(String response) {
        ...
        try {
            ...
            if (response.toLowerCase().contains("true")) {
                ...
                Intent intent = new Intent(mchformula.this, MainActivity2.class);
    ...
    private void calculateMCH() {
        // Get Hb and RBC Count inputs
        String hbString = HbInput.getText().toString().trim();
        String rbcCountString = rbcCountInput.getText().toString().trim();

        // Check if inputs are not empty
        if (!hbString.isEmpty() && !rbcCountString.isEmpty()) {
            // Parse inputs to double
            double hb = Double.parseDouble(hbString);
            double rbcCount = Double.parseDouble(rbcCountString);

            // Calculate MCH using the formula
            double mch = (hb / rbcCount) * 10;
            x = String.format("%.2f", mch);

            // Display the result with units "pg"
            String formattedResult = String.format("%.2f pg", mch);
            ...
            outputText.setText("Please enter values for Hb and RBC Count");
